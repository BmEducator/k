...
import '../Models/mistakesModel.dart';
...
class quiz_ExamMode_Screen extends StatefulWidget {
  ...
  String mode;
   quiz_ExamMode_Screen({Key? key,required this.mode,required this.completedQuizes,required this.quiz, required this.isLanguageTranslation,required this.isShuffle,required this.selecteddataList}) : super(key: key);
...
class _quiz_ExamMode_ScreenState extends State<quiz_ExamMode_Screen> {
  ...
  _showFinishDialog() {
    ...
    bool isLoading= false;
    List<QuestionModel> tempList = [] ;
    List<QuestionModel> correctList = [] ;

    for(int i= 0;i<=correctOptions.length-1;i++ ) {
      if (selectedOptions[i] != correctOptions[i]) {
        print("wr00");

        mistakesQuestioList.add(allQuestionListinEnglish[i].statement
            + "~" + allQuestionListinEnglish[i].answer);

        tempList.add(QuestionModel(
            image: allQuestionListinEnglish[i].image,
            statement: allQuestionListinEnglish[i].statement,
            optionA: allQuestionListinEnglish[i].optionA,
            option2: allQuestionListinEnglish[i].option2,
            optionC: allQuestionListinEnglish[i].optionC,
            answer: allQuestionListinEnglish[i].answer, optionD: allQuestionListinEnglish[i].optionD));
      }
      else {
        print("corr");
        correct = correct + 1;
        correctList.add(QuestionModel(
            image: allQuestionListinEnglish[i].image,
            statement: allQuestionListinEnglish[i].statement,
            optionA: allQuestionListinEnglish[i].optionA,
            option2: allQuestionListinEnglish[i].option2,
            ...
            return  StatefulBuilder(
              ...
              return WillPopScope(
                ...
                questions.forEach((element) {
                  ...
                  tempList.removeWhere((element) => element.statement == q.statement);
                  tempList.add(q);
                });

                print("all ${tempList.length}");

                correctList.forEach((e) {
                  tempList.removeWhere((element) => element.statement == e.statement);

                });

                print("correct ${tempList.length}");
                ...
                mistakesListModel m =
                mistakesListModel(questions: tempList );
                await FirebaseFirestore.instance.collection("students").
                doc(email).collection("allMistakes").doc("mistakes")
                    .set(m.toMap());
                             }
                             else{
                               mistakesListModel m =
  ...
  Future<void> saveData() async {
    ...
    quizModel q = quizModel(
        ...
        mistakes: mistakesQuestioList, mode: widget.mode);
    ...
    doc(email).collection("quizesProgress").doc("quizes")
        .set({q.id : list}, SetOptions(merge: true));
